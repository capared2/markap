...
def _collect_sitemap(fetcher: Fetcher, url: str, seen: set[str], depth: int, out: set[str]) -> None:
    ...
    for child in children:
        _collect_sitemap(fetcher, child, seen, depth + 1, out)
...
def from_sitemaps(fetcher: Fetcher, extra: list[str] | None = None) -> set[str]:
    found: set[str] = set()
    visited: set[str] = set()
    ...
    for root in dict.fromkeys(roots):
        _collect_sitemap(fetcher, root, visited, 0, found)
...
def from_feeds(fetcher: Fetcher, feeds: list[str] | None = None) -> set[str]:
    found: set[str] = set()
...
def crawl(fetcher: Fetcher, max_depth: int = 1, seeds: list[str] | None = None) -> set[str]:
...
def discover(fetcher: Fetcher, sources: list[str], crawl_depth: int = 1) -> set[str]:
    """Run every requested discovery source and merge the results."""
    found: set[str] = set()
    if "sitemap" in sources:
        found |= from_sitemaps(fetcher)
    if "rss" in sources:
        found |= from_feeds(fetcher)
    if "crawl" in sources:
        found |= crawl(fetcher, max_depth=crawl_depth)
